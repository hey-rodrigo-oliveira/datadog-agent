...
func (e *engine) IngestLog(source string, l *logObs) []advanceRequest {
	...
	for _, extractor := range e.extractors {
		...
		for _, m := range out.Metrics {
			...
			// Only active after freeze with muting enabled — mutedHashes is also the
			// accumulator during the window, so checking frozen prevents dropping
			// virtual metrics mid-window while detection is still running.
			if e.baseline != nil && e.baseline.frozen && e.baseline.config.MuteNoisyMetrics && len(e.baseline.mutedHashes) > 0 {
				sortedTags := canonicalizeTags(tags)
				if _, ok := e.baseline.mutedHashes[seriesKeyHash(extractor.Name(), m.Name, sortedTags)]; ok {
					continue
				}
				tags = sortedTags
			}
