...
type Server struct {
	...
	instanceID    atomic.Value // string; set once from /run body
...
func (s *Server) aliveCheckReady(w http.ResponseWriter) {
	...
	log.Warn("MicroVM lifecycle: /ready called but child is not alive (not yet started or already exited); returning 503")
	w.WriteHeader(http.StatusServiceUnavailable)
}
